...
import { getNodeEvaluator } from "./node-registry.js";

/**
 * Evaluate a behavior tree against a person model.
 * Returns the first successful action, template, or escalation signal.
 */
export async function evaluateBT(
  node: BehaviorNode,
  model: PersonModel,
  userMessage?: string,
  blackboard: Record<string, any> = {},
): Promise<EvalResult> {
  return await evalNode(node, model, userMessage, [], blackboard);
}
  ...
  switch (node.type) {
    ...
    case "selector": {
      ...
      for (let i = 0; i < node.children.length; i++) {
        const result = await evalNode(node.children[i], model, userMessage, [...trace, `sel[${i}]`], blackboard);
    ...
    case "sequence": {
      ...
      for (let i = 0; i < node.children.length; i++) {
        ...
        const result = await evalNode(child, model, userMessage, [...trace, `seq[${i}]`], blackboard);
    ...
    case "weighted_random": {
      ...
      for (const choice of node.choices) {
        r -= choice.weight;
        if (r <= 0) {
          return await evalNode(choice.child, model, userMessage, [...trace, `wrnd`], blackboard);
        }
      }
      // Fallback to last choice
      const last = node.choices[node.choices.length - 1];
      return await evalNode(last.child, model, userMessage, [...trace, `wrnd:last`], blackboard);
    }
    ...
    case "skill": {
      ...
      if (skill) {
        ...
        return await evalNode(skill.tree, model, userMessage, [...trace, `skill:${node.name}`], blackboard);
      }
    ...
    default: {
      // Check for registered plugin node types
      const pluginEvaluator = getNodeEvaluator((node as any).type);
      if (pluginEvaluator) {
        return await pluginEvaluator(node, model, userMessage, trace, evalNode, blackboard);
      }
