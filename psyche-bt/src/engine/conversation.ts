/**
 * Conversation Engine — The main interaction loop.
 *
 * For each user message:
 *   1. Analyze message (topics, entities, emotional state) — lightweight
 *   2. Evaluate behavior tree against person model
 *   3a. If BT handles it → fill template via Flash Lite → respond
 *   3b. If BT escalates → reason via Flash → respond → compile pattern
 *   4. Update person model with new information
 *   5. Record result for stats tracking
 */
...
export async function processTurn(
  ...
  let btResult = evaluateBT(model.policy.tree!, model, userMessage);
