/**
 * Behavioral Benchmark: Office incident where the engineer MUST use a CLI coding agent.
 *
 * Purpose:
 * - Simulated agents keep doing orchestration/planning/coordination
 * - But actual code edits are delegated to a CLI coding agent (gemini CLI) and applied as a patch
 *
 * Requirements:
 * - OFFICE_TOOLS_ALLOW_SHELL=1
 * - OFFICE_TOOLS_ALLOW_GEMINI_CLI=1
 * - OFFICE_TOOLS_ALLOW_GIT_APPLY=1
 * - GOOGLE_GENERATIVE_AI_API_KEY set (planner LLM)
 *
 * Run:
 *   OFFICE_TOOLS_ALLOW_SHELL=1 OFFICE_TOOLS_ALLOW_GEMINI_CLI=1 OFFICE_TOOLS_ALLOW_GIT_APPLY=1 npx tsx src/behavioral-tests/61-office-incident-cli-coder-benchmark.ts
 */
...
function parseArgs(): { outDir?: string; maxTicks: number } {
  ...
  for (const raw of process.argv.slice(2)) {
    ...
    if (raw.startsWith("--maxTicks=")) maxTicks = Math.max(50, Math.min(5000, Number(raw.slice("--maxTicks=".length)) || 260));
  }
...
async function main(): Promise<void> {
  ...
  const runId = `office-incident-cli-coder-${Date.now()}`;
  ...
  const boardType = `kanban_board_device_cli_${unique}`;
  ...
  const world = createArgosWorld("OfficeIncidentCliCoderBenchmark") as any;
  ...
  executeActions(
    world as any,
    [
      { eid: admin, action: { type: "interact", target: "Workstation", content: 'init_workspace_fixture {"fixtureId":"office_incident_cli_1"}' } },
      // This board is a computer with kanban traits in the office tool layer; initialize kanban columns.
      { eid: admin, action: { type: "interact", target: "Team Board", content: 'kanban_init {"project":"Incident","columns":["Backlog","In Progress","Review","Done"]}' } },
      {
        ...
        action: {
          ...
          target: "Team Board",
          content:
            'kanban_upsert_card {"title":"[ENG] Fix invoice discount math (CLI)","column":"Backlog","description":"UseCLI: true\\nBug: discounted invoices are wrong. Fix the code by delegating to a CLI coding agent.\\nConstraints:\\n- You MUST use gemini_cli to produce a unified diff (starting with `diff --git`).\\n- Then apply it with git_apply_from_last_gemini src/math.cjs src/service.cjs\\n- Do NOT use replace_in_file for code paths in this ticket.\\n- Do NOT edit ci.cjs or test.cjs.\\nWrites: src/math.cjs, src/service.cjs\\nRun: node ci.cjs"}',
        },
      },
      {
        eid: admin,
        action: {
          ...
          target: "Team Board",
          content:
            'kanban_upsert_card {"title":"[QA] Verify CI and close","column":"Backlog","description":"DependsOn: [ENG] Fix invoice discount math (CLI)\\nRun: node ci.cjs"}',
        },
      },
    ] as any,
    registry as any
  ...
  const score = Math.max(0, Math.round(100 - toolFailures * 10 - Math.max(0, durationMs / 1000 - 90) * 0.2));
  ...
  console.log(`✓ office incident (CLI coder) benchmark complete. Output: ${outDir}`);

  assert(isCardInColumn(world, "Team Board", "[ENG] Fix invoice discount math (CLI)", "Done"), "expected ENG ticket to be Done");
