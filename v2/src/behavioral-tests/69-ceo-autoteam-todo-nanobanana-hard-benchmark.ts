/**
 * Behavioral Benchmark: Hard benchmark: CEO autoteams a product org to ship a branded Todo App with multiple Nano Banana assets + a narrative QA report.
 *
 * Goal:
 ...
 *
 * Run:
 *   OFFICE_TOOLS_ALLOW_SHELL=1 npx tsx src/behavioral-tests/69-ceo-autoteam-todo-nanobanana-hard-benchmark.ts
 */
...
async function main() {
  ...
  const runId = `ceo-autoteam-nanobanana-hard-${Date.now()}`;
  ...
  Goal.description[missionGoal] =
    ...
    "(A) [DESIGN] Generate Brand Assets (Nano Banana) — Create public/assets/logo.png and public/assets/hero.png using the Workstation's `generate_image` and `edit_image` affordances (Nano Banana). " +
    ...
    "(D) [QA] Verify + Report — depends on ENG + PM. Run CI and write a narrative QA report at docs/qa-report.md (not a checklist). Include sections: Summary, What I tested, Results, Suggestions. Minimum ~200 words. " +
  ...
  let describeImageToolResults = 0;
  let editImageToolResults = 0;
  ...
  for (let tick = 0; tick < args.maxTicks; tick++) {
    ...
    for (const s of stimuli) {
      ...
      if (s.type === "tool_result" && typeof s.content === "string" && s.content.startsWith("[Tool:nano_banana.edit_image]")) {
        editImageToolResults++;
      }
  ...
  assert(describeImageToolResults >= 2, "expected at least two describe_image calls (logo + hero, plus optional re-check)");
  assert(editImageToolResults >= 1, "expected at least one edit_image iteration on a Nano Banana asset");

  const wsRoot = ensureOfficeDeviceSandboxDir(world as any, workstation);
  const heroAbs = path.join(wsRoot, "public", "assets", "hero.png");
  assert(fs.existsSync(heroAbs), "expected public/assets/hero.png to exist in the workspace");
  assert(fs.statSync(heroAbs).size > 100, "expected hero.png to be non-trivial (>100 bytes)");

  const htmlAbs = path.join(wsRoot, "public", "index.html");
  const html = fs.readFileSync(htmlAbs, "utf8");
  assert(html.includes("/assets/logo.png"), "expected index.html to reference /assets/logo.png");
  assert(html.includes("/assets/hero.png"), "expected index.html to reference /assets/hero.png");

  const qaAbs = path.join(wsRoot, "docs", "qa-report.md");
  assert(fs.existsSync(qaAbs), "expected docs/qa-report.md to be created by QA");
  const qa = fs.readFileSync(qaAbs, "utf8");
  const qaWords = qa.trim().split(/\s+/).filter(Boolean);
  assert(qaWords.length >= 200, `expected QA report to be ~200+ words (got ${qaWords.length})`);
  const qaLower = qa.toLowerCase();
  for (const h of ["summary", "what i tested", "results", "suggestions"]) {
    assert(qaLower.includes(h), `expected QA report to include section: ${h}`);
  }

  assert(imageAssetStimuli > 0, "expected at least one image_asset visual stimulus broadcast");
  console.log(`✓ CEO autoteam Nano Banana HARD todo benchmark passed. Output: ${outDir}`);
}
